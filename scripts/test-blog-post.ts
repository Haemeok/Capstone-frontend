...
const FAKE_RECIPE_GAJI: Recipe = {
  ...
  steps: [
    {
      stepNumber: 1,
      instruction: "가지와 부추를 흐르는 물에 씻습니다",
      action: "손질하기",
      stepImageUrl: "",
      stepImageKey: null,
    },
    {
      stepNumber: 2,
      instruction: "가지 꼭지를 떼고 길게 칼집을 넣습니다",
      action: "썰기",
      stepImageUrl: "",
      stepImageKey: null,
    },
    {
      stepNumber: 3,
      instruction: "물 2000ml를 끓이고 소금 8큰술을 넣어 불을 끕니다",
      action: "끓이기",
      stepImageUrl: "",
      stepImageKey: null,
    },
    {
      stepNumber: 4,
      instruction: "뜨거운 소금물에 가지를 넣고 5분간 데칩니다",
      action: "데치기",
      stepImageUrl: "",
      stepImageKey: null,
    },
    {
      stepNumber: 5,
      instruction: "무를 채칼로 얇게 채썰어 준비합니다",
      action: "채썰기",
      stepImageUrl: "",
      stepImageKey: null,
    },
    {
      stepNumber: 6,
      instruction: "채썬 무에 설탕을 넣고 5분간 절입니다",
      action: "버무리기",
      stepImageUrl: "",
      stepImageKey: null,
    },
    ...
    },
    {
      stepNumber: 8,
      instruction:
        "무에 다진 양파, 부추, 고춧가루, 멸치액젓, 깨, 생강가루를 넣고 버무려 양념을 만듭니다",
      action: "버무리기",
      stepImageUrl: "",
      stepImageKey: null,
    },
    {
      stepNumber: 9,
      instruction: "데친 가지의 물기를 손으로 짭니다",
      action: "손질하기",
      stepImageUrl: "",
      stepImageKey: null,
    },
    {
      stepNumber: 10,
      instruction: "물기 짠 가지 칼집 사이에 양념을 채웁니다",
      action: "무치기",
      stepImageUrl: "",
      stepImageKey: null,
    },
    {
      stepNumber: 11,
      instruction: "용기에 담아 서늘한 곳에서 숙성시킵니다",
      action: "담그기",
      stepImageUrl: "",
      stepImageKey: null,
    },
...
const autoScore = (post: BlogPost) => {
  ...
  const stepBodyAvg =
    post.steps.reduce((sum, s) => sum + s.body.length, 0) / post.steps.length;
  ...
  const checks = [
    ...
    {
      id: "A2",
      desc: `서술 본문 ≥ 1500자 (실측 ${narrative})`,
      pass: narrative >= 1500,
    },
    {
      id: "A3",
      desc: `step body 평균 ≥ 80자 (실측 ${Math.round(stepBodyAvg)})`,
      pass: stepBodyAvg >= 80,
    },
    {
      id: "A4",
      desc: `kitchenTips 2~4개 (실측 ${post.kitchenTips.length})`,
      pass: post.kitchenTips.length >= 2 && post.kitchenTips.length <= 4,
    },
    {
      id: "A5",
      desc: `appliedKnowledge 250~600자 (실측 ${post.appliedKnowledge.length})`,
      pass:
        post.appliedKnowledge.length >= 250 &&
        post.appliedKnowledge.length <= 620,
    },
    {
      id: "A6",
      desc: `평균치 표현 미검출 (검출 ${avgWords.length}: ${avgWords.join(", ") || "-"})`,
      pass: avgWords.length === 0,
    },
    {
      id: "A7",
      desc: `금지어 미검출 (검출 ${bannedTone.length}: ${bannedTone.join(", ") || "-"})`,
      pass: bannedTone.length === 0,
    },
    {
      id: "A8",
      desc: `lead 정량 후크는 원가/시간/kcal만 (오류 ${leadMisplaced.length}: ${leadMisplaced.join(", ") || "-"})`,
      pass: leadMisplaced.length === 0,
    },
    {
      id: "A9",
      desc: `closing 마지막 보관·주의 노트 미검출 (검출 ${closingHygieneTail.length}: ${closingHygieneTail.join(", ") || "-"})`,
      pass: closingHygieneTail.length === 0,
    },
  ];
...
const main = async () => {
  ...
  const slots = [
    ...recipe.steps.map((s) => `step-${s.stepNumber}`),
    "final-plated",
  ];
  ...
  fs.writeFileSync(
    file,
    JSON.stringify(
      { seeds: { lead: leadSeed.id, closing: closingSeed.id }, post },
      null,
      2
    )
  );
  ...
  console.log();
  console.log(
    `▣ steps (총 ${post.steps.length}개, 평균 ${Math.round(stepBodyAvg)}자):`
  );
