...
const generateNewKeywords = (): TaggedKeyword[] => {
  ...
  for (const ing of ingredients) {
    for (const method of methods) {
      keywords.push({
        keyword: `${ing} ${method} 만들기`,
        strategy: "ing_situation_method",
      });
      keywords.push({
        keyword: `간단 ${ing} ${method}`,
        strategy: "ing_situation_method",
      });
      keywords.push({
        keyword: `${ing} ${method} 황금레시피`,
        strategy: "ing_situation_method",
      });
    }
  }
  ...
  keywords.push(
    ...trending.map((kw) => ({ keyword: kw, strategy: "trending" }))
  );
  ...
  keywords.push(
    ...substitutions.map((kw) => ({ keyword: kw, strategy: "substitution" }))
  );

  // ── 5. "~로 만드는" 패턴 ──
  for (const ing of ingredients) {
    keywords.push({
      keyword: `${ing}로 만드는 반찬`,
      strategy: "making_pattern",
    });
    keywords.push({
      keyword: `${ing}로 만드는 한끼`,
      strategy: "making_pattern",
    });
    keywords.push({
      keyword: `${ing} 하나로 만드는 요리`,
      strategy: "making_pattern",
    });
    keywords.push({
      keyword: `${ing} 대량소비 레시피`,
      strategy: "making_pattern",
    });
    keywords.push({
      keyword: `${ing} 활용 레시피`,
  ...
  for (let i = 0; i < pairIngredients.length; i++) {
    for (let j = i + 1; j < pairIngredients.length; j++) {
      keywords.push({
        keyword: `${pairIngredients[i]} ${pairIngredients[j]} 요리`,
        strategy: "pair_expansion",
      });
      keywords.push({
        keyword: `${pairIngredients[i]} ${pairIngredients[j]} 레시피`,
        strategy: "pair_expansion",
      });
    }
  }
...
const main = async () => {
  ...
  const defaultAllowlist: AllowlistData = {
    generatedAt: "",
    stats: { totalActive: 0, addedThisCycle: 0, promotedFromImmature: 0 },
    pages: [],
  };
  const allowlist: AllowlistData =
    safeReadJson<AllowlistData>(ALLOWLIST_PATH) ?? defaultAllowlist;
  ...
  const newKeywords = allKeywords.filter(
    ({ keyword }) => !existingQs.has(keyword)
  );
  ...
  for (let i = 0; i < newKeywords.length; i += CONCURRENCY) {
    ...
    const eta = Math.round(
      (((Date.now() - startTime) / checked) * (newKeywords.length - checked)) /
        1000
    );
  ...
  console.log(`\n=== 결과 ===`);
  console.log(
    `사이트맵: ${allowlist.pages.length - addedCount} → ${allowlist.pages.length} (+${addedCount})`
  );
  ...
  console.log("\n=== 전략별 전환율 ===");
  for (const [strategy, stat] of Object.entries(strategyStats).sort(
    ([, a], [, b]) => b.active - a.active
  )) {
    const rate =
      stat.generated > 0
        ? ((stat.active / stat.generated) * 100).toFixed(1)
        : "0.0";
    console.log(
      `  ${strategy.padEnd(25)} ${String(stat.generated).padStart(6)} 생성 → ${String(stat.active).padStart(5)} ACTIVE (${rate}%)`
    );
  }
