...
import styles from "./CookingRecordBoard.module.css";
...
export type CookingRecordBoardProps = {
  ...
  onAddRecord: () => void;
  onShareRecord: () => void;
  renderPhoto?: (record: CookingRecordStickerItem) => ReactNode;
  actionsPosition?: "fixed" | "contained";
  showRecordMeta?: boolean;
  showRecordNames?: boolean;
  children?: ReactNode;
};

export const CookingRecordBoard = ({
  ariaLabel,
  records,
  background,
  recordCountLabel,
    ...
    >
      {records.map((record) => (
        ...
        >
          {renderPhoto ? (
            renderPhoto(record)
          ) : record.displayRecord?.displayMode === "DISH" ? (
