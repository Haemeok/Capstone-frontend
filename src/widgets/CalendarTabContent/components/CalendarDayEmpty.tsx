import { triggerHaptic } from "@/shared/lib/bridge";
import { cn } from "@/shared/lib/utils";

type CalendarDayEmptyProps = {
  dateNumber: number;
  isToday: boolean;
  addRecordLabel?: string;
  onAddRecord?: () => void;
};

export const CalendarDayEmpty = ({
  dateNumber,
  isToday,
  addRecordLabel,
  onAddRecord,
}: CalendarDayEmptyProps) => {
  const dateContent = (
    <span
      className={cn(
        "text-ink-sub absolute top-1 left-1/2 -translate-x-1/2 text-xs",
        isToday && "text-olive-dark font-semibold"
      )}
    >
      {dateNumber}
      {isToday ? (
        <span
          data-testid="calendar-day-today-dot"
          aria-hidden="true"
          className="bg-olive-dark absolute top-4 left-1/2 h-1 w-1 -translate-x-1/2 rounded-full"
        />
      ) : null}
    </span>
  );

  if (addRecordLabel && onAddRecord) {
    const handleAddRecord = () => {
      triggerHaptic("Light");
      onAddRecord();
    };

    return (
      <td className="relative h-full w-full">
        <button
          type="button"
          aria-label={addRecordLabel}
          onClick={handleAddRecord}
          className="focus-visible:ring-olive-dark relative h-full min-h-11 w-full cursor-pointer focus-visible:ring-2 focus-visible:outline-none focus-visible:ring-inset"
        >
          {dateContent}
        </button>
      </td>
    );
  }

  return <td className="relative h-full w-full">{dateContent}</td>;
};
