...
const HomeQuickNav = ({ locale, messages }: HomeQuickNavProps) => (
  <nav
    ...
    className="w-full border-b-[8px] border-gray-100 bg-white pt-[18px] md:border-b-0"
  >
          ...
          <li key={item.id} className="min-w-0">
            ...
            >
              <span
                ...
                className="flex h-[60px] w-[60px] shrink-0 items-center justify-center overflow-hidden rounded-[18px] bg-gray-100 transition-colors active:bg-gray-200 md:h-[72px] md:w-[72px] md:rounded-[20px]"
              >
                <Image
                  src={item.imageSrc}
                  alt=""
                  lazy={false}
                  fit="contain"
                  wrapperClassName="h-[60px] w-[60px] bg-transparent md:h-[72px] md:w-[72px]"
                  skeletonClassName="bg-transparent"
                  imgClassName={cn(
                    item.isYoutube ? "p-2.5" : "p-0.5",
                    item.imageClassName
                  )}
                />
              </span>
              <span
                aria-hidden="true"
                className="text-ink max-w-full text-[11.5px] leading-[1.3] font-medium tracking-[-0.04em] whitespace-nowrap max-[340px]:text-[10px] md:text-[13px]"
              >
                {compactLabel ? (
                  <>
    ...
    <Link
      ...
      className="text-ink flex min-h-11 w-full cursor-pointer items-center justify-center gap-0.5 border-t border-gray-200 px-3 text-[15px] no-underline transition-colors active:bg-gray-50"
    >
      <span className="font-medium">{messages.trendMore}</span>
