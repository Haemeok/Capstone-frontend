    ...
    >
      {imageUrl ? (
        <Image
          ...
          aspectRatio="1 / 1"
          priority={priority}
          wrapperClassName="w-full bg-beige"
          imgClassName="transition-opacity duration-200 group-hover:opacity-90"
        />
      ) : (
        <div className="aspect-square w-full bg-beige" aria-hidden />
      )}
      {category && (
      ...
      <h3 className="mt-1 line-clamp-2 text-sm font-semibold text-brown sm:text-[15px]">
        {title}
      </h3>
    </Link>
  );
