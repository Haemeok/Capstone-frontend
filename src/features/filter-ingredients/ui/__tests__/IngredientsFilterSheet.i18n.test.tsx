...
it("T-05: ja 경로에서 타이틀/placeholder가 ja로 렌더된다", async () => {
  renderSheet();
  expect(await screen.findByText("材料から探す")).toBeInTheDocument();
...
it("T-07: 선택 2개면 적용 버튼이 ja이고 {count}=2로 치환된다", async () => {
  ...
  expect(await screen.findByText("2件で探す")).toBeInTheDocument();
});

it("T-09: ko 경로에서는 기존 한글이 그대로다(회귀)", async () => {
  pathnameMock.mockReturnValue("/search");
  renderSheet();
  expect(await screen.findByText("재료로 검색하기")).toBeInTheDocument();
