...
export const useJobPolling = () => {
  ...
  const pendingJobs = useYoutubeImportStoreV2(useShallow(pendingJobsSelector));
  ...
  useEffect(() => {
    ...
    const pollAllJobs = async () => {
      ...
      try {
        const currentPendingJobs = pendingJobsSelector(
          useYoutubeImportStoreV2.getState()
        );
        await Promise.all(
  ...
  }, [isVisible, pendingJobs, deps]);

  return {
    pendingJobCount: pendingJobs.length,
    isPolling: isVisible && pendingJobs.length > 0,
  };
};
