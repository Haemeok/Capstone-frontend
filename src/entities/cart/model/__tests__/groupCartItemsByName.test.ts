...
it("같은 이름 + 숫자 수량 + 동일 단위면 총량을 합산한다 (T-55)", () => {
  const groups = groupCartItemsByName([
...
it("소수 합산은 부동소수점 잔재 없이 표기한다 (T-55)", () => {
  const groups = groupCartItemsByName([
...
it("단위가 비어 있으면 숫자 수량도 합산하지 않는다 (T-56)", () => {
  const groups = groupCartItemsByName([
    item({ cartItemId: "c1", quantity: "1", unit: "" }),
    item({ cartItemId: "c2", quantity: "2", unit: "" }),
  ]);
