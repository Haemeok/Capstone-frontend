...
const getLocaleConfig = (locale: Locale): LocaleConfig => {
  if (locale === "ja") {
    return {
      title: `recipio - 5万+ 家庭料理レシピ・AIおすすめ`,
      description: `YouTubeリンクで保存、5万件以上の家庭料理。AIおすすめと、ホームパーティー・記念日・ダイエットまでシーン別レシピを一か所で。`,
      ogLocale: "ja_JP",
  ...
  if (locale === "en") {
    return {
      title: `recipio - 50,000+ home recipes · AI picks`,
      description: `Save recipes from YouTube links. 50,000+ home recipes with AI recommendations and recipes for parties, anniversaries, and diets.`,
      ogLocale: "en_US",
