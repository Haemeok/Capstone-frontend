...
export const useImageTest = () => {
  ...
  const runOne = useCallback(
    async (modelId: string, prompt: string, signal: AbortSignal) => {
      ...
      try {
        ...
        setResults((prev) => ({
          ...prev,
          [modelId]: { status: "error", message, latencyMs: 0 },
        }));
      }
    },
    []
  ...
  const runAll = useCallback(
    async (modelIds: string[], prompt: string) => {
      ...
      setResults(
        Object.fromEntries(
          validIds.map((id) => [id, { status: "pending" as const }])
        )
      );

      await Promise.allSettled(
        validIds.map((id) => runOne(id, prompt, controller.signal))
      );
      setRunning(false);
    },
    [runOne]
