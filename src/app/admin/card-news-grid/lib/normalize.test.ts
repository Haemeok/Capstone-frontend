...
const genItem = (over?: Partial<{ dishName: string; caption: string; imagePrompt: string }>) => ({
  dishName: "불고기",
  caption: "간장 5:설탕 2",
...
describe("normalizeItems", () => {
  ...
  it("상한 초과 글자수는 truncate된다 (검증 탈락 대신 보정)", () => {
    ...
    const items = [genItem({ caption: longCaption, dishName: longName }), ...nineItems().slice(1)];
  ...
  it("9개 초과는 9개로 자른다", () => {
    const res = normalizeItems({ header: "h", items: Array.from({ length: 12 }, () => genItem()) });
    expect(res.items).toHaveLength(9);
  });
  ...
  it("9개 미만이면 throw", () => {
    expect(() => normalizeItems({ header: "h", items: nineItems().slice(0, 7) })).toThrow();
  });
});
...
describe("normalizeTopics", () => {
  ...
  it("5개 초과는 5개로 자른다", () => {
    const res = normalizeTopics({ topics: Array.from({ length: 8 }, () => genTopic()) });
    expect(res.topics).toHaveLength(5);
  });

  it("긴 title/concept은 truncate된다", () => {
    const res = normalizeTopics({
      topics: [{ title: "가".repeat(MAX_HEADER + 10), concept: "나".repeat(80) }],
    });
