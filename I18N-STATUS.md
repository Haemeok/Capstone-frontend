...
| 레시피북    | `/recipe-books`, `/recipe-books/[bookId]` | M     | 🟡   | 목록(`/recipe-books`)·그리드/카드 chrome + 미러 라우트 완료(S4 `46766ef0`). **상세(`/[bookId]`)+편집/이동/삭제/이름변경/생성 시트+엔티티 에러/검증+책내 레시피 데이터 lang 진행중**. 데이터 lang은 요청에만(키 X, §6 결정) |
