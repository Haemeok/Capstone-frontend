...
| 캘린더      | `/calendar/[date]`, `/calendar/timeline` | L     | 🟡   | ja/en 거의 완료(`useUserPagesDict().calendar` — 타임라인/날짜/영양/스트릭). **잔여 2건:** ① `SavingsCard`(KRW 절약 `원`/`절약` 하드코딩, locale 게이트 없음 → 프로필 `MonthlySavingsSummary` 패턴대로 ja/en 숨김 권장) ② `[date]/error.tsx` 한글. **2026-06-16 실측 🔴→🟡** |
