...
- [Query invalidation vs cache patch](rules/policy-query-invalidation.md) — invalidate refetches every loaded page (storm); trim infinite pages before membership-changing refetches
- [Multi-stage mutation state](rules/policy-multistage-mutation-state.md) — put preparation in its own mutation, retry only the final request, and expose one coherent status
...
- [Headless mobile viewport](rules/test-headless-mobile-viewport.md) — Windows headless Chromium may crop a small `--window-size` while keeping a ~500px CSS viewport; set CDP device metrics and verify `innerWidth` before judging responsive screenshots
