---
name: brainstorming
description: "Project-local override. You MUST use this before any creative work - creating features, building components, adding functionality, or modifying behavior. Explores user intent, requirements and design before implementation. Prefer this over superpowers:brainstorming in this project."
---
...
- **Acceptance Criteria (required):** before finishing, restate the core requirements as observable, testable success conditions — "When `<condition>`, the system `<observable outcome>`." These become the input to the test-design step. Dig out implicit ones (error cases, the requirement hiding inside a vague request). Reject implementation statements ("uses helper X") — criteria describe what the user can observe.
