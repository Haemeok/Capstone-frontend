...
- `dispatch-commit-pathspec-bypasses-partial-index` — After partial staging, do not use `git commit -- <shared-file>`: the commit pathspec can select the whole working-tree file. Audit the complete index file list, then commit the index without a pathspec.
- `dispatch-lint-staged-defeats-partial-staging` — If the installed pre-commit hook re-adds whole formatted files instead of hiding unstaged hunks, partial staging is lost. Verify the actual hook behavior; use remove-commit-restore only for the whole-file-restaging case.
