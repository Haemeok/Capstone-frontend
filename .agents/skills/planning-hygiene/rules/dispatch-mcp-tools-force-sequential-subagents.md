...
Key points:
- MCP server scope is the **Codex session**, not the subagent. The subagent inherits the parent's MCP connections; they aren't a fresh per-subagent server.
...
- Any `mcp__*` namespace is a signal. Treat it as bucket B until proven otherwise.
- Pure subagent work (extracting labels from a local codebase with `rg` and focused file reads, generating JSON, writing files in disjoint paths) is safely parallel. Don't over-correct by serializing everything.
