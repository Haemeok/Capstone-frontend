...
### 앱 설치 유도 클릭 (app_open_click) 일별·store별

`app_open_click`은 앱 설치 유도 클릭 이벤트. 발화 지점 2곳 — `StoreBadges`(store=`app_store`/`google_play` + locale)와 `SmartAppBannerCard`(locale만, **store 파라미터 없음 → store가 `null`인 행 = 스마트 앱 배너 클릭**).

```sql
SELECT event_date,
       (SELECT value.string_value FROM UNNEST(event_params) WHERE key='store') AS store,
       COUNT(*) AS clicks, COUNT(DISTINCT user_pseudo_id) AS users
FROM {events}
WHERE _TABLE_SUFFIX BETWEEN '20260730' AND '20260805'
  AND event_name='app_open_click'
GROUP BY event_date, store ORDER BY event_date, store
```

### 특정 이벤트가 어느 페이지에서 발생했는지 (path 분해)

```sql
SELECT REGEXP_EXTRACT((SELECT value.string_value FROM UNNEST(event_params) WHERE key='page_location'), r'https?://[^/]+(/[^?#]*)') AS path,
       COUNT(*) AS clicks
FROM {events}
WHERE _TABLE_SUFFIX BETWEEN '20260627' AND '20260805' AND event_name='app_open_click'
GROUP BY path ORDER BY clicks DESC
```

검증된 사실: store 파라미터 있는 `app_open_click`(=StoreBadges)은 100% `/landing`에서만 발생. 2026-08-03 랜딩 우회 배포(`20de9a35`) 후 랜딩 PV가 일 ~150 → ~14로 붕괴, store 클릭 0.
