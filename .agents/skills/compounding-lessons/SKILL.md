...
description: Persist a verified, generalizable debugging or workflow lesson into an existing project skill so future Codex agents do not rediscover it. Use when the user asks to compound, save, remember, or add a lesson for other agents; after a non-obvious multi-iteration fix; and as the mandatory final step after finishing-a-development-branch, subagent-driven-development, or executing-plans. Self-report when there is nothing worth capturing.
---

# Compounding Lessons

This skill turns a conversation's hard-won debugging wins into persistent knowledge files that seed future conversations. Each session shouldn't have to re-discover the same root cause; if it took an hour of iteration here, the next Codex should arrive already knowing.

The output of this skill is a rule file (or an update to one) that reads like a rule — not a diary entry. If a future Codex can't internalize a generalizable principle from what you write, the compounding failed.
...
Do NOT use for:
- Project-specific facts (go in `AGENTS.md`)
...
5. **Principle** — the generalizable rule a future Codex should internalize, imperative mood.
...
Scan both `.agents/skills/` (project-local) and `~/.agents/skills/` (user-global). Match the lesson to an existing skill using these signals:
...
| LLM API / AI SDK usage | `llm-integration/` |
...
If no existing skill fits, create a new project-local skill under `.agents/skills/<topic>/` with a `SKILL.md` + `rules/` structure (model it on `vercel-react-best-practices`). Pick this path only when the topic genuinely doesn't belong anywhere else — fragmented knowledge is worse than centralized knowledge.

**Important: don't modify plugin-cached skills** (files under `~/.codex/plugins/cache/...`). Those get overwritten on plugin updates. Either find the project-local copy (`.agents/skills/<name>/`) or create a sibling rule in a project-local skill.
...
This matters because the SKILL.md body is what Codex sees at skill-load time; rule files themselves are read on demand. A rule that isn't listed in SKILL.md might never get discovered.
...
<1-2 lines on what future Codex will now know without having to rediscover it.>
```
...
- **Generalize ruthlessly.** Future Codex won't have this project's code. Strip project-specific names (component names, color tokens, file paths) unless they clarify the concept. Use placeholder names (`Label`, `Component`, `MyButton`) in code examples.
...
- **Explain the why.** An imperative without mechanism becomes dogma and gets ignored. Always name the underlying behavior (e.g., "Motion re-measures each frame when animating to `auto`") so future Codex can judge edge cases the rule didn't anticipate.
...
- **Editing plugin-cached skills.** Those get overwritten. Write to the project-local `.agents/skills/` copy or create a new project-local skill.
...
- Could a Codex with zero context about this project read the rule and apply it correctly on the first try? If not, generalize more.
